...
const ItemDetail = ({ item }) => {
  ...
  const { data: itm } = useSWR(
    `https://equipment-renting.herokuapp.com/furniture/item/${item._id}`,
    fetcher,
...
export const getStaticProps = async (context) => {
  const res = await fetch(
    `https://equipment-renting.herokuapp.com/furniture/item/${context.params.id[1]}`
  );
...
export const getStaticPaths = async () => {
  ...
  const items = funzone.map(({ _id, name }) => [
    `hire-${name.replace(/\s/g, "-")}-at-kalyanagar`,
    _id,
  ]);

  const paths = items.map((id) => ({
    params: { id: [id[0].toString(), id[1].toString()] },
  }));
